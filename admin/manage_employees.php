...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['add_employee'])) {
        ...
        try {
            ...
            $stmt = $conn->prepare("
                INSERT INTO users (name, email, password, role, position, base_salary, manager_id) 
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ");
            $result = $stmt->execute([$name, $email, $password, $role, $position, $base_salary, $manager_id]);
            
            if ($result) {
                $user_id = $conn->lastInsertId();
                
                // Calculate allowances based on base salary
                $transport_allowance = $base_salary * 0.05; // 5% of base salary
                $meal_allowance = $base_salary * 0.05; // 5% of base salary
                $health_allowance = $base_salary * 0.05; // 5% of base salary
                $position_allowance = $base_salary * 0.10; // 10% of base salary
                $attendance_allowance = $base_salary * 0.02; // 2% of base salary
                $family_allowance = $base_salary * 0.05; // 5% of base salary
                $communication_allowance = $base_salary * 0.02; // 2% of base salary
                $education_allowance = $base_salary * 0.05; // 5% of base salary

                // Insert allowances
                $stmt = $conn->prepare("
                    INSERT INTO allowances (
                        user_id, transport_allowance, meal_allowance, health_allowance,
        ...
        try {
            ...
            if (!empty($_POST['password'])) {
                ...
                $stmt = $conn->prepare("
                    ...
                    WHERE id = ?
                ");
                $result = $stmt->execute([$name, $email, $password, $role, $position, $base_salary, $manager_id, $id]);
            } else {
                $stmt = $conn->prepare("
                    UPDATE users 
                    ...
                    WHERE id = ?
                ");
                $result = $stmt->execute([$name, $email, $role, $position, $base_salary, $manager_id, $id]);
            }

            if ($result) {
                // Calculate new allowances based on updated base salary
                $transport_allowance = $base_salary * 0.05;
                $meal_allowance = $base_salary * 0.05;
                $health_allowance = $base_salary * 0.05;
                $position_allowance = $base_salary * 0.10;
                $attendance_allowance = $base_salary * 0.02;
                $family_allowance = $base_salary * 0.05;
                $communication_allowance = $base_salary * 0.02;
                $education_allowance = $base_salary * 0.05;

                // Update allowances
                $stmt = $conn->prepare("
                    UPDATE allowances 
                    SET transport_allowance = ?, meal_allowance = ?, health_allowance = ?,
                        position_allowance = ?, attendance_allowance = ?,
                        family_allowance = ?, communication_allowance = ?, education_allowance = ?
                    WHERE user_id = ?
                ");
...
<div class="main-container">
            ...
            <div class="form-grid">
                ...
                <div class="form-field">
                    <label for="edit_password">
                        <i class="fas fa-lock"></i> Password <small>(kosongkan jika tidak ingin
                            mengubah)</small>
                    </label>
                    <div class="input-wrapper password-wrapper">
                ...
                <div class="form-field" id="edit-manager-field">
                    ...
                    <div class="input-wrapper select-wrapper">
                        <select name="manager_id" id="edit_manager_id" class="form-control">
                            <option value="">-- Pilih Manager --</option>
                            <?php foreach ($managers as $manager): ?>
                                <option value="<?php echo $manager['id']; ?>"><?php echo $manager['name']; ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                        <span class="focus-border"></span>
                        ...
                        <tbody>
                            ...
                            ?>
                                <tr data-role="<?php echo $employee['role']; ?>">
                                    ...
                                    <td class="money">
                                        <a href="#" onclick="showAllowanceDetails(<?php echo htmlspecialchars(json_encode($employee), ENT_QUOTES, 'UTF-8'); ?>)" class="allowance-link">
    ...
    <div id="allowance-details-modal" class="modal-form" style="display: none;">
        <div class="modal-header">
        ...
        <div class="modal-content">
            <div class="allowance-details">
                <div class="employee-info">
                    <h4 id="allowance-employee-name"></h4>
                    <p id="allowance-employee-position"></p>
                </div>
                <div class="allowance-grid">
                    <div class="allowance-item">
                        <span class="allowance-label">Tunjangan Transportasi</span>
                        <span class="allowance-value" id="transport-allowance"></span>
                    </div>
                    <div class="allowance-item">
                        <span class="allowance-label">Tunjangan Makan</span>
                        <span class="allowance-value" id="meal-allowance"></span>
                    </div>
                    <div class="allowance-item">
                        <span class="allowance-label">Tunjangan Kesehatan</span>
                        <span class="allowance-value" id="health-allowance"></span>
                    </div>
                    <div class="allowance-item">
                        <span class="allowance-label">Tunjangan Jabatan</span>
                        <span class="allowance-value" id="position-allowance"></span>
                    </div>
                    <div class="allowance-item">
                        <span class="allowance-label">Tunjangan Kehadiran</span>
                        <span class="allowance-value" id="attendance-allowance"></span>
                    </div>
                    <div class="allowance-item" style="display:none">
                        <span class="allowance-label">Tunjangan Hari Raya (THR)</span>
                        <span class="allowance-value" id="thr-allowance"></span>
                    </div>
                    <div class="allowance-item">
                        <span class="allowance-label">Tunjangan Keluarga</span>
                        <span class="allowance-value" id="family-allowance"></span>
                    </div>
                    <div class="allowance-item">
                        <span class="allowance-label">Tunjangan Komunikasi</span>
                        <span class="allowance-value" id="communication-allowance"></span>
                    </div>
                    <div class="allowance-item">
                        <span class="allowance-label">Tunjangan Pendidikan</span>
                        <span class="allowance-value" id="education-allowance"></span>
                    </div>
                </div>
                <div class="allowance-total">
                    <span class="total-label">Total Tunjangan</span>
                    <span class="total-value" id="total-allowance"></span>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    function showAllowanceDetails(employee) {
        document.getElementById('allowance-employee-name').textContent = employee.name;
        document.getElementById('allowance-employee-position').textContent = employee.position || '-';
        
        document.getElementById('transport-allowance').textContent = formatMoney(employee.transport_allowance);
        document.getElementById('meal-allowance').textContent = formatMoney(employee.meal_allowance);
        document.getElementById('health-allowance').textContent = formatMoney(employee.health_allowance);
        document.getElementById('position-allowance').textContent = formatMoney(employee.position_allowance);
        document.getElementById('attendance-allowance').textContent = formatMoney(employee.attendance_allowance);
        document.getElementById('family-allowance').textContent = formatMoney(employee.family_allowance);
        document.getElementById('communication-allowance').textContent = formatMoney(employee.communication_allowance);
        document.getElementById('education-allowance').textContent = formatMoney(employee.education_allowance);
        
        const totalAllowance = parseFloat(employee.transport_allowance) +
                              parseFloat(employee.meal_allowance) +
                              parseFloat(employee.health_allowance) +
                              parseFloat(employee.position_allowance) +
                              parseFloat(employee.attendance_allowance) +
                              parseFloat(employee.family_allowance) +
                              parseFloat(employee.communication_allowance) +
                              parseFloat(employee.education_allowance);
        
        document.getElementById('total-allowance').textContent = formatMoney(totalAllowance);
        
        document.getElementById('allowance-details-modal').style.display = 'flex';
        document.getElementById('modal-overlay').style.display = 'block';
        document.body.style.overflow = 'hidden';
    }

    function hideAllowanceDetails() {
        document.getElementById('allowance-details-modal').style.display = 'none';
        document.getElementById('modal-overlay').style.display = 'none';
        document.body.style.overflow = 'auto';
    }
