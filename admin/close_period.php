...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['close_period'])) {
    ...
    try {
        ...
        $stmt = $conn->prepare("
            ...
            WHERE month = ? AND year = ? AND status = 'finalized'
        ");
        $stmt->execute([$month, $year]);
        $closed_count = $stmt->fetchColumn();
        
        if ($closed_count > 0) {
            set_alert('warning', 'This period has already been closed');
        } else {
            // Update all payroll records for this period to 'finalized'
            $stmt = $conn->prepare("
                UPDATE payroll 
            ...
            set_alert('success', 'Payroll period has been closed successfully');
        }
...
<div class="container">
    <div class="page-header">
        <h2><i class="fas fa-calendar-times"></i> Tutup Periode Payroll</h2>
    </div>
    
    <div class="close-period card">
        <div class="card-header">
            <h3><i class="fas fa-lock"></i> Tutup Periode Baru</h3>
        </div>
        <div class="card-body">
            <form method="post" action="" class="form-grid">
                ...
                <div class="form-group">
                    <button type="submit" name="close_period" class="btn-primary">
    ...
    <div class="periods-list card">
        <div class="card-header">
            <h3><i class="fas fa-history"></i> Riwayat Periode</h3>
        </div>
        <div class="card-body">
